...
## Clean, immersive game-feel UI without web emojis.
...
const DEBUG_MENU_SCENE := "res://scenes/main_menu.tscn"

const FONT_TITLE := "res://assets/Fonts/Long_Cang,Rubik_Glitch/Rubik_Glitch/RubikGlitch-Regular.ttf"
...
func _build_ui() -> void:
	...
	grid_node.set_anchors_preset(PRESET_FULL_RECT)
	grid_node.mouse_filter = MOUSE_FILTER_IGNORE
	add_child(grid_node)
	...
	left_col.custom_minimum_size = Vector2(480, 0)
	...
	title_lbl.text = "EMBER DEPARTURE"
	title_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	if ResourceLoader.exists(FONT_TITLE):
		var tf := load(FONT_TITLE) as Font
		if tf != null:
			title_lbl.add_theme_font_override("font", tf)
	...
	subtitle_lbl.text = "灰 烬 · 极限追缉"
	subtitle_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	if ResourceLoader.exists(FONT_CHINESE):
		var cf := load(FONT_CHINESE) as Font
		if cf != null:
			subtitle_lbl.add_theme_font_override("font", cf)
	subtitle_lbl.add_theme_font_size_override("font_size", 32)
	subtitle_lbl.modulate = Color(0.9, 0.92, 0.95, 0.9)
	left_col.add_child(subtitle_lbl)

	var tag_lbl := Label.new()
	tag_lbl.text = "1v1 智能动力学逃生与追捕对决"
	...
	btn_editor.pressed.connect(func() -> void: get_tree().change_scene_to_file(MAP_EDITOR_SCENE))
	...
	var btn_debug := _create_menu_button("开发者调试沙盒 (Dev Sandbox)", "res://assets/UI_assets/gear-hammer.svg", Color(0.5, 0.55, 0.65))
	btn_debug.pressed.connect(func() -> void: get_tree().change_scene_to_file(DEBUG_MENU_SCENE))
	right_col.add_child(btn_debug)
...
func _on_start_game_pressed() -> void:
	...
	# Save selected map to global config / autoload or instantiate scene
